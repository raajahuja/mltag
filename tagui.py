...
if os.path.isfile('mltAg_python.txt'): os.remove('mltAg_python.txt')
...
def _tagui_output():
    ...
    init_directory_output_file = os.path.join(_tagui_init_directory, 'mltAg_python.txt')

    # sleep to not splurge cpu cycles in while loop
    while not os.path.isfile('mltAg_python.txt'):
    ...
    if os.path.isfile('mltAg_python.txt'):
        tagui_output_file = _py23_open('mltAg_python.txt', 'r')
        tagui_output_text = _py23_read(tagui_output_file.read())
        tagui_output_file.close()
        os.remove('mltAg_python.txt')
    else:
...
def _python_flow():
    ...
    flow_text = '// MLTAG EXECUTION \r\n\r\nlive'
    flow_file = _py23_open('mltAg_python', 'w')
...
def _tagui_local():
    ...
    javascript_file = _py23_open('mltAg_local.js', 'w')
...
def _tagui_delta(base_directory = None):
    ...
    if os.path.isfile(base_directory + '/' + 'mltAg_python_' + __version__): return True
    ...
    delta_done_file = _py23_open(base_directory + '/' + 'mltAg_python_' + __version__, 'w')
    delta_done_file.write(_py23_write('mltAg'))
...
def unzip(file_to_unzip = None, unzip_location = None):
    ...
    if file_to_unzip is None or file_to_unzip == '':
        print('[mltAg][ERROR] - filename missing for unzip()')
        return False
    elif not os.path.isfile(file_to_unzip):
        print('[mltAg][ERROR] - file specified missing for unzip()')
        return False
...
def setup():
    ...
    # print('[mltAg][INFO] - setting up TagUI for use in your Python environment')
    ...
    if platform.system() == 'Linux': tagui_zip_file = 'TagUI_Linux.zip'
        ...
        print('[mltAg][ERROR] - unknown ' + platform.system() + ' operating system to setup mltAg')
        return False

    print('[mltAg][INFO] - We are making things ready for you. Please wait...')
    ...
    tagui_zip_url = 'https://github.com/raajahuja/mltag/releases/download/v1.0/' + tagui_zip_file
    ...
    if not os.path.isfile(home_directory + '/' + 'tagui' + '/' + 'src' + '/' + 'tagui'):
        print('[mltAg][ERROR] - unable to unzip mltAg to ' + home_directory)
        return False
    ...
    # print('[mltAg][INFO] - mltAg ready to use now.')
    ...
    if platform.system() == 'Linux':
        ...
        if os.system('chmod -R 755 ' + tagui_directory + ' > /dev/null 2>&1') != 0:
            print('[mltAg][ERROR] - unable to set permissions for .tagui folder')
            return False 

        # check that php, a dependency for tagui, is installed and working
        if os.system('php --version > /dev/null 2>&1') != 0:
            print('[mltAg][INFO] - PHP is not installed by default on your Linux distribution')
            print('[mltAg][INFO] - google how to install PHP (eg for Ubuntu, apt-get install php)')
            print('[mltAg][INFO] - after that, mltAg ready for use in your Python environment')
            return False

        else:
            print('[mltAg][INFO] - mltAg now ready for use in your local environment')
    ...
    if platform.system() == 'Darwin':
        ...
        if not os.path.isfile('/usr/local/opt/openssl/lib/libssl.1.0.0.dylib'):
            ...
            if not os.path.isfile('/usr/local/opt/openssl/lib/libssl.1.0.0.dylib'):
                print('')
                print('[mltAg][INFO] - now installing OpenSSL dependency using Homebrew')
                print('[mltAg][INFO] - you may be prompted for login password to continue')
                print('')
                ...
                if not os.path.isfile('/usr/local/opt/openssl/lib/libssl.1.0.0.dylib'):
                    print('[mltAg][INFO] - OpenSSL was not able to be installed automatically')
                    print('[mltAg][INFO] - run below 2 commands in your terminal to install manually')
                    print('[mltAg][INFO] - after that, mltAg ready for use in your local environment')
                    print('')
    ...
    if platform.system() == 'Windows':
        ...
        if os.system(tagui_directory + '/' + 'src' + '/' + 'php/php.exe -v > nul 2>&1') != 0:
            print('[mltAg][INFO] - now installing missing Visual C++ Redistributable dependency')
            ...
            if os.system(tagui_directory + '/' + 'src' + '/' + 'php/php.exe -v > nul 2>&1') != 0:
                print('[mltAg][INFO] - MSVCR110.dll is still missing, install vcredist_x86.exe from')
                print('[mltAg][INFO] - https://www.microsoft.com/en-us/download/details.aspx?id=30679')
                print('[mltAg][INFO] - after that, mltAg ready for use in your Python environment')
                return False
...
def init(visual_automation = False, chrome_browser = True):
    ...
    if _tagui_started:
        print('[mltAg][ERROR] - use close() before using init() again')
        return False
    ...
    if platform.system() == 'Windows' and ' ' in os.getcwd():
        print('[mltAg][INFO] - to use mltAg for Python on Windows, avoid space in folder path name')
        return False
    ...
    if visual_automation:
        ...
        if os.system('java -version ' + shell_silencer) != 0:
            print('[mltAg][INFO] - for visual automation mode, Java JDK v8 (64-bit) or later is required')
            print('[mltAg][INFO] - to use visual automation feature, download Java JDK v8 (64-bit) from below')
            print('[mltAg][INFO] - https://www.oracle.com/technetwork/java/javase/downloads/jdk8-downloads-2133151.html')
            return False
        else:
    ...
    tagui_cmd = tagui_executable + ' mltAg_python ' + browser_option
    # print(tagui_cmd)
    ...
    try:
        ...
        while True:
            ...
            if _process.poll() is not None:
                print('[mltAg][ERROR] - following happens when starting mltAg...')
                print('')
                os.system(tagui_cmd)
                print('')
                _tagui_visual = False
            ...
            if 'LIVE MODE - type done to quit' in tagui_out:
                ...
                _tagui_write('echo "[mltAg][STARTED]"\n')
                _tagui_write('echo "[mltAg][' + str(_tagui_id) + '] - listening for inputs"\n')
                ...
                if not _tagui_started:
                    print('[mltAg][ERROR] - mltAg process ended unexpectedly')
                    return False

                # remove generated tagui flow, js code and custom functions files
                if os.path.isfile('mltAg_python'): os.remove('mltAg_python')
                if os.path.isfile('mltAg_python.js'): os.remove('mltAg_python.js')
                if os.path.isfile('mltAg_python.raw'): os.remove('mltAg_python.raw')
                if os.path.isfile('mltAg_local.js'): os.remove('mltAg_local.js')
...
def _ready():
    ...
    try:
        ...
        if tagui_out.strip().startswith('[mltAg][') and tagui_out.strip().endswith('] - listening for inputs'):
            return True
        else:
            return False
...
def send(tagui_instruction = None):
    ...
    if not _tagui_started:
        print('[mltAg][ERROR] - use init() before using send()')
        return False
    ...
    try:
        # failsafe exit if tagui process gets killed for whatever reason
        if _process.poll() is not None:
            print('[mltAg][ERROR] - no active mltAg process to send()')
            _tagui_visual = False
            _tagui_chrome = False
            _tagui_started = False
            return False
        ...
        _tagui_write('echo "[mltAg][' + str(_tagui_id) + '] - ' + echo_safe_instruction + '"\n')
...
def close():
    ...
    if not _tagui_started:
        print('[mltAg][ERROR] - use init() before using close()')
        return False

    try:
        # failsafe exit if tagui process gets killed for whatever reason
        if _process.poll() is not None:
            print('[mltAg][ERROR] - no active mltAg process to close()')
            _tagui_visual = False
            _tagui_chrome = False
            _tagui_started = False
            return False
        ...
        _tagui_write('echo "[mltAg][FINISHED]"\n')
        _tagui_write('done\n')
        ...
        # to handle user changing current directory after init() is called
        if os.path.isfile(os.path.join(_tagui_init_directory, 'mltAg_python')):
            os.remove(os.path.join(_tagui_init_directory, 'mltAg_python'))
        if os.path.isfile(os.path.join(_tagui_init_directory, 'mltAg_python.js')):
            os.remove(os.path.join(_tagui_init_directory, 'mltAg_python.js'))
        if os.path.isfile(os.path.join(_tagui_init_directory, 'mltAg_python.raw')):
            os.remove(os.path.join(_tagui_init_directory, 'mltAg_python.raw'))
        if os.path.isfile(os.path.join(_tagui_init_directory, 'mltAg_local.js')):
            os.remove(os.path.join(_tagui_init_directory, 'mltAg_local.js'))   

        # remove generated tagui log and data files if not in debug mode
        if not debug():
            if os.path.isfile('mltAg_python.log'): os.remove('mltAg_python.log')
            if os.path.isfile('mltAg_python.txt'): os.remove('mltAg_python.txt')
        
            # to handle user changing current directory after init() is called
            if os.path.isfile(os.path.join(_tagui_init_directory, 'mltAg_python.log')):
                os.remove(os.path.join(_tagui_init_directory, 'mltAg_python.log'))
            if os.path.isfile(os.path.join(_tagui_init_directory, 'mltAg_python.txt')):
                os.remove(os.path.join(_tagui_init_directory, 'mltAg_python.txt'))
...
def exist(element_identifier = None):
    if not _started():
        print('[mltAg][ERROR] - use init() before using exist()')
        return False
    ...
    if element_identifier.lower() in ['page.png', 'page.bmp']:
        if _visual():
            ...
            print('[mltAg][ERROR] - page.png / page.bmp requires init(visual_automation = True)')
            return False
    ...
    if element_identifier.lower().endswith('.png') or element_identifier.lower().endswith('.bmp'):
        if not os.path.isfile(element_identifier):
            print('[mltAg][ERROR] - missing image file ' + element_identifier)
            return False
        elif not _visual():
            print('[mltAg][ERROR] - ' + element_identifier + ' identifier requires init(visual_automation = True)')
            return False
        ...
        if len(element_identifier.split(',')) in [2, 3]:
            if not any(c.isalpha() for c in element_identifier):
                if _visual():
                    ...
                    print('[mltAg][ERROR] - x, y coordinates require init(visual_automation = True)')
                    return False

    send('exist_result = exist(\'' + _sdq(element_identifier) + '\').toString()')
    send('dump exist_result to mltAg_python.txt')
...
def url(webpage_url = None):
    if not _started():
        print('[mltAg][ERROR] - use init() before using url()')
        return False

    if not _chrome():
        print('[mltAg][ERROR] - url() requires init(chrome_browser = True)')
        return False
    ...
    if webpage_url is not None and webpage_url != '':
        if webpage_url.startswith('http://') or webpage_url.startswith('https://'):
            ...
            print('[mltAg][ERROR] - URL does not begin with http:// or https:// ')
            return False

    else:
        send('dump url() to mltAg_python.txt')
        url_result = _tagui_output()
        return url_result

def click(element_identifier = None, test_coordinate = None):
    if not _started():
        print('[mltAg][ERROR] - use init() before using click()')
        return False

    if element_identifier is None or element_identifier == '':
        print('[mltAg][ERROR] - target missing for click()')
        return False
    ...
    if not exist(element_identifier):
        print('[mltAg][ERROR] - cannot find ' + element_identifier)
        return False
...
def rclick(element_identifier = None, test_coordinate = None):
    if not _started():
        print('[mltAg][ERROR] - use init() before using rclick()')
        return False

    if element_identifier is None or element_identifier == '':
        print('[mltAg][ERROR] - target missing for rclick()')
        return False
...
def dclick(element_identifier = None, test_coordinate = None):
    if not _started():
        print('[mltAg][ERROR] - use init() before using dclick()')
        return False

    if element_identifier is None or element_identifier == '':
        print('[mltAg][ERROR] - target missing for dclick()')
        return False
...
def hover(element_identifier = None, test_coordinate = None):
    if not _started():
        print('[mltAg][ERROR] - use init() before using hover()')
        return False

    if element_identifier is None or element_identifier == '':
        print('[mltAg][ERROR] - target missing for hover()')
        return False
...
def type(element_identifier = None, text_to_type = None, test_coordinate = None):
    if not _started():
        print('[mltAg][ERROR] - use init() before using type()')
        return False

    if element_identifier is None or element_identifier == '':
        print('[mltAg][ERROR] - target missing for type()')
        return False

    if text_to_type is None or text_to_type == '':
        print('[mltAg][ERROR] - text missing for type()')
        return False
...
def select(element_identifier = None, option_value = None, test_coordinate1 = None, test_coordinate2 = None):
    if not _started():
        print('[mltAg][ERROR] - use init() before using select()')
        return False

    if element_identifier is None or element_identifier == '':
        print('[mltAg][ERROR] - target missing for select()')
        return False

    if option_value is None or option_value == '':
        print('[mltAg][ERROR] - option value missing for select()')
        return False

    if element_identifier.lower() in ['page.png', 'page.bmp'] or option_value.lower() in ['page.png', 'page.bmp']:
        print('[mltAg][ERROR] - page.png / page.bmp identifiers invalid for select()')
        return False
    ...
    if option_value.lower().endswith('.png') or option_value.lower().endswith('.bmp'):
        if not os.path.isfile(option_value):
            print('[mltAg][ERROR] - missing image file ' + option_value)
            return False
        elif not _visual():
            print('[mltAg][ERROR] - ' + option_value + ' identifier requires init(visual_automation = True)')
            return False
...
def read(element_identifier = None, test_coordinate1 = None, test_coordinate2 = None, test_coordinate3 = None):
    if not _started():
        print('[mltAg][ERROR] - use init() before using read()')
        return ''

    if element_identifier is None or element_identifier == '':
        print('[mltAg][ERROR] - target missing for read()')
        return ''
    ...
    if element_identifier.lower() != 'page' and not exist(element_identifier):
        ...
        send('dump read_result to mltAg_python.txt')
...
def snap(element_identifier = None, filename_to_save = None, test_coord1 = None, test_coord2 = None, test_coord3 = None):
    if not _started():
        print('[mltAg][ERROR] - use init() before using snap()')
        return False

    if element_identifier is None or element_identifier == '':
        print('[mltAg][ERROR] - target missing for snap()')
        return False

    if filename_to_save is None or filename_to_save == '':
        print('[mltAg][ERROR] - filename missing for snap()')
        return False
...
def load(filename_to_load = None):
    if filename_to_load is None or filename_to_load == '':
        print('[mltAg][ERROR] - filename missing for load()')
        return ''

    elif not os.path.isfile(filename_to_load):
        print('[mltAg][ERROR] - cannot load file ' + filename_to_load)
        return ''
...
def dump(text_to_dump = None, filename_to_save = None):
    if text_to_dump is None:
        print('[mltAg][ERROR] - text missing for dump()')
        return False

    elif filename_to_save is None or filename_to_save == '':
        print('[mltAg][ERROR] - filename missing for dump()')
        return False
...
def write(text_to_write = None, filename_to_save = None):
    if text_to_write is None:
        print('[mltAg][ERROR] - text missing for write()')
        return False

    elif filename_to_save is None or filename_to_save == '':
        print('[mltAg][ERROR] - filename missing for write()')
        return False
...
def keyboard(keys_and_modifiers = None):
    if not _started():
        print('[mltAg][ERROR] - use init() before using keyboard()')
        return False

    if keys_and_modifiers is None or keys_and_modifiers == '':
        print('[mltAg][ERROR] - keys to type missing for keyboard()')
        return False

    if not _visual():
        print('[mltAg][ERROR] - keyboard() requires init(visual_automation = True)')
        return False
...
def mouse(mouse_action = None):
    if not _started():
        print('[mltAg][ERROR] - use init() before using mouse()')
        return False

    if mouse_action is None or mouse_action == '':
        print('[mltAg][ERROR] - \'down\' / \'up\' missing for mouse()')
        return False

    if not _visual():
        print('[mltAg][ERROR] - mouse() requires init(visual_automation = True)')
        return False
...
def table(element_identifier = None, filename_to_save = None):
    if not _started():
        print('[mltAg][ERROR] - use init() before using table()')
        return False

    if element_identifier is None or element_identifier == '':
        print('[mltAg][ERROR] - target missing for table()')
        return False

    elif filename_to_save is None or filename_to_save == '':
        print('[mltAg][ERROR] - filename missing for table()')
        return False
...
def check(condition_to_check = None, text_if_true = '', text_if_false = ''):
    if condition_to_check is None:
        print('[mltAg][ERROR] - condition missing for check()')
        return False
...
def upload(element_identifier = None, filename_to_upload = None):
    if not _started():
        print('[mltAg][ERROR] - use init() before using upload()')
        return False

    if element_identifier is None or element_identifier == '':
        print('[mltAg][ERROR] - target missing for upload()')
        return False

    elif filename_to_upload is None or filename_to_upload == '':
        print('[mltAg][ERROR] - filename missing for upload()')
        return False
...
def download(download_url = None, filename_to_save = None):
    ...
    if download_url is None or download_url == '':
        print('[mltAg][ERROR] - download URL missing for download()')
        return False
    ...
    try:
        ...
        print('[mltAg][ERROR] - failed downloading from ' + download_url + '...')
        print(str(e))
        return False
    ...
    if os.path.isfile(filename_to_save):
        ...
        print('[mltAg][ERROR] - failed downloading to ' + filename_to_save)
        return False

def frame(main_frame = None, sub_frame = None):
    if not _started():
        print('[mltAg][ERROR] - use init() before using frame()')
        return False

    if not _chrome():
        print('[mltAg][ERROR] - frame() requires init(chrome_browser = True)')
        return False
    ...
    if not exist(frame_identifier):
        print('[mltAg][ERROR] - cannot find frame with @name or @id as \'' + main_frame + '\'')
        return False
    ...
    if sub_frame is not None and sub_frame != '':
        ...
        if not exist(frame_identifier):
            print('[mltAg][ERROR] - cannot find sub frame with @name or @id as \'' + sub_frame + '\'')
            return False
...
def popup(string_in_url = None):
    if not _started():
        print('[mltAg][ERROR] - use init() before using popup()')
        return False

    if not _chrome():
        print('[mltAg][ERROR] - popup() requires init(chrome_browser = True)')
        return False
    ...
    send('dump chrome_targetid to mltAg_python.txt')
    popup_result = _tagui_output()
    if popup_result != '':
        return True
    else:
        print('[mltAg][ERROR] - cannot find popup tab containing URL string \'' + string_in_url + '\'')
        return False

def api(url_to_query = None):
    print('[mltAg][INFO] - although mltAg supports calling APIs with headers and body,')
    print('[mltAg][INFO] - recommend using requests package with lots of online docs')
    return True

def run(command_to_run = None):
    if command_to_run is None or command_to_run == '':
        print('[mltAg][ERROR] - command(s) missing for run()')
        return ''
...
def dom(statement_to_run = None):
    if not _started():
        print('[mltAg][ERROR] - use init() before using dom()')
        return ''

    if statement_to_run is None or statement_to_run == '':
        print('[mltAg][ERROR] - statement(s) missing for dom()')
        return ''

    if not _chrome():
        print('[mltAg][ERROR] - dom() requires init(chrome_browser = True)')
        return ''

    else:
        send('dom ' + statement_to_run)
        send('dump dom_result to mltAg_python.txt')
        dom_result = _tagui_output()
        return dom_result

def vision(command_to_run = None):
    if not _started():
        print('[mltAg][ERROR] - use init() before using vision()')
        return False

    if command_to_run is None or command_to_run == '':
        print('[mltAg][ERROR] - command(s) missing for vision()')
        return False

    if not _visual():
        print('[mltAg][ERROR] - vision() requires init(visual_automation = True)')
        return False
...
def timeout(timeout_in_seconds = None):
    if not _started():
        print('[mltAg][ERROR] - use init() before using timeout()')
        return False
...
def present(element_identifier = None):
    if not _started():
        print('[mltAg][ERROR] - use init() before using present()')
        return False
    ...
    send('dump present_result to mltAg_python.txt')
    if _tagui_output() == 'true':
        return True
    else:
        return False
...
def count(element_identifier = None):
    if not _started():
        print('[mltAg][ERROR] - use init() before using count()')
        return int(0)
    ...
    if not _chrome():
        print('[mltAg][ERROR] - count() requires init(chrome_browser = True)')
        return int(0)

    send('count_result = count(\'' + _sdq(element_identifier) + '\').toString()')
    send('dump count_result to mltAg_python.txt')
    return int(_tagui_output())

def title():
    if not _started():
        print('[mltAg][ERROR] - use init() before using title()')
        return ''

    if not _chrome():
        print('[mltAg][ERROR] - title() requires init(chrome_browser = True)')
        return ''

    send('dump title() to mltAg_python.txt')
    title_result = _tagui_output()
    return title_result

def text():
    if not _started():
        print('[mltAg][ERROR] - use init() before using text()')
        return ''

    if not _chrome():
        print('[mltAg][ERROR] - text() requires init(chrome_browser = True)')
        return ''

    send('dump text() to mltAg_python.txt')
    text_result = _tagui_output()
    return text_result

def timer():
    if not _started():
        print('[mltAg][ERROR] - use init() before using timer()')
        return float(0)

    send('dump timer() to mltAg_python.txt')
    timer_result = _tagui_output()
    return float(timer_result)

def mouse_xy():
    if not _started():
        print('[mltAg][ERROR] - use init() before using mouse_xy()')
        return ''

    if not _visual():
        print('[mltAg][ERROR] - mouse_xy() requires init(visual_automation = True)')
        return ''

    send('dump mouse_xy() to mltAg_python.txt')
    mouse_xy_result = _tagui_output()
    return mouse_xy_result

def mouse_x():
    if not _started():
        print('[mltAg][ERROR] - use init() before using mouse_x()')
        return int(0)

    if not _visual():
        print('[mltAg][ERROR] - mouse_x() requires init(visual_automation = True)')
        return int(0)

    send('dump mouse_x() to mltAg_python.txt')
    mouse_x_result = _tagui_output()
    return int(mouse_x_result)

def mouse_y():
    if not _started():
        print('[mltAg][ERROR] - use init() before using mouse_y()')
        return int(0)

    if not _visual():
        print('[mltAg][ERROR] - mouse_y() requires init(visual_automation = True)')
        return int(0)

    send('dump mouse_y() to mltAg_python.txt')
    mouse_y_result = _tagui_output()
    return int(mouse_y_result)

def clipboard(text_to_put = None):
    if not _started():
        print('[mltAg][ERROR] - use init() before using clipboard()')
        return False

    if not _visual():
        print('[mltAg][ERROR] - clipboard() requires init(visual_automation = True)')
        return False

    if text_to_put is None:
        send('dump clipboard() to mltAg_python.txt')
